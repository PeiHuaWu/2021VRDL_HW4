...
from util import utils_image as utils_image
...
def main():
    ...
    for idx, path in enumerate(sorted(glob.glob(os.path.join(folder, '*')))):
        ...
        cv2.imwrite(f'{save_dir}/{imgname}_pred.png', output)  # output: xxx_pred.png
        ...
        if img_gt is not None:
            ...
            psnr = utils_image.calculate_psnr(output, img_gt, border=border)
            ssim = utils_image.calculate_ssim(output, img_gt, border=border)
            test_results['psnr'].append(psnr)
            test_results['ssim'].append(ssim)
            if img_gt.ndim == 3:  # RGB image
                output_y = utils_image.bgr2ycbcr(output.astype(np.float32) / 255.) * 255.
                img_gt_y = utils_image.bgr2ycbcr(img_gt.astype(np.float32) / 255.) * 255.
                psnr_y = utils_image.calculate_psnr(output_y, img_gt_y, border=border)
                ssim_y = utils_image.calculate_ssim(output_y, img_gt_y, border=border)
                test_results['psnr_y'].append(psnr_y)
                test_results['ssim_y'].append(ssim_y)
            if args.task in ['jpeg_car']:
                psnr_b = utils_image.calculate_psnrb(output, img_gt, border=border)
